...
var save_method;
var table;
//Load Datatables Services
const _loadDtServices = () => {
    table = $('#dt-services').DataTable({
        searchDelay: 300,
        processing: true,
        serverSide: true,
        ajax: {
            url: base_url+ 'api/manage_institutionservices/show',
        ...
        fnDrawCallback: function (settings, display) {
            ...
            //Search Table
            $("#search-dtServices").on("keyup", function () {
                table.search(this.value).draw();
                if ($(this).val().length > 0) {
                    $("#clear-searchDtServices").show();
                } else {
                    $("#clear-searchDtServices").hide();
                }
            });
            //Clear Search Table
            $("#clear-searchDtServices").on("click", function () {
                $("#search-dtServices").val(""),
                table.search("").draw(),
                $("#clear-searchDtServices").hide();
            });
            //Custom Table
            $("#dt-services_length select").selectpicker(),
...
const _closeCard = (card) => {
    if(card=='form_service') {
        save_method = '';
        _clearFormService(), $('#card-formService .card-header .card-title').html('');
    }
    $('#card-formService').hide(), $('#card-dtServices').show();
}
//Clear Form Layanan
const _clearFormService = () => {
    $("#form-service")[0].reset(), _loadSummernote('Isi deskripsi layanan ...', 450, '#card-formService', '#description'), $('#description').summernote('code', ''),
    _loadDropifyFile('', '#thumb');
    if (save_method == "" || save_method == "add_service") {
...
//Add Layanan
const _addService = () => {
    save_method = "add_service";
    _clearFormService(),
    $("#card-formService .card-header .card-title").html(
        `<h3 class="fw-bolder fs-2 text-gray-900"><i class="bi bi-window-plus fs-2 text-gray-900 me-2"></i>Form Tambah Layanan</h3>`
    ),
    $("#card-dtServices").hide(), $("#card-formService").show();
}
//Edit Layanan
const _editService = (idp) => {
    save_method = "update_service";
    $('#form-service')[0].reset(), _loadSummernote('Isi deskripsi layanan ...', 450, '#card-formService', '#description'), $('#description').summernote('code', ''),
...
//Save Layanan by Enter
$("#form-service input").keyup(function (event) {
...
$("#btn-save").on("click", function (e) {
    ...
    if (name.val() == '') {
        toastr.error('Nama layanan masih kosong...', 'Uuppss!', {"progressBar": true, "timeOut": 1500});
        name.focus();
        $('#btn-save').html('<i class="las la-save fs-1 me-3"></i>Simpan').attr('disabled', false);
        return false;
    } if (description.summernote('isEmpty')) {
        toastr.error('Isi deskripsi layanan masih kosong...', 'Uuppss!', {"progressBar": true, "timeOut": 1500});
        description.summernote('focus');
        $('#btn-save').html('<i class="las la-save fs-1 me-3"></i>Simpan').attr('disabled', false);
        return false;
    } if (thumb_preview == '') {
        toastr.error('Thumbnail/ gambar layanan masih kosong...', 'Uuppss!', {"progressBar": true, "timeOut": 1500});
    ...
    Swal.fire({
        ...
        if (result.value) {
            let target = document.querySelector("#card-formService"), blockUi = new KTBlockUI(target, { message: messageBlockUi });
            blockUi.block(), blockUi.destroy();
            let formData = new FormData($("#form-service")[0]), ajax_url = base_url+ "api/manage_institutionservices/store";
            if(save_method == 'update_service') {
                ajax_url = base_url+ "api/manage_institutionservices/update";
            }
            $.ajax({
                url: ajax_url,
                headers: { 'X-CSRF-Token': $('meta[name="csrf-token"]').attr('content') },
                ...
                success: function (data) {
                    ...
                    if (data.status == true) {
                        Swal.fire({
                            ...
                            _closeCard('form_service'), _loadDtServices();
                        });
                    } else {
                        Swal.fire({
                            title: "Ooops!",
...
const _updateStatus = (idp, value) => {
    ...
    Swal.fire({
        ...
        if (result.value) {
            let target = document.querySelector('#card-dtServices'), blockUi = new KTBlockUI(target, { message: messageBlockUi });
            blockUi.block(), blockUi.destroy();
            // Load Ajax
            $.ajax({
                url: base_url+ "api/manage_institutionservices/update_status",
                headers: { 'X-CSRF-Token': $('meta[name="csrf-token"]').attr('content') },
                type: "POST",
                dataType: "JSON",
                data: {
...
const _deleteService = (idp) => {
    Swal.fire({
        ...
        if (result.value) {
            ...
            // Load Ajax
            $.ajax({
                url: base_url+ "api/manage_institutionservices/delete",
                headers: { 'X-CSRF-Token': $('meta[name="csrf-token"]').attr('content') },
                type: "POST",
                dataType: "JSON",
                data: {
